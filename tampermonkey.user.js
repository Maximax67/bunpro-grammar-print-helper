// ==UserScript==
// @name         Bunpro Grammar Print Helper
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  Adds a print button to Bunpro grammar point pages
...
(function() {
    ...
    printButton.style.fontSize = '22px';
    printButton.style.backgroundColor = 'transparent';
    printButton.style.color = '#4CAF50';
    printButton.style.border = 'none';
    printButton.style.cursor = 'pointer';

    let scale = parseFloat(localStorage.getItem('printScale')) || 0.75;

    const scaleDiv = document.createElement('div');
    scaleDiv.style.position = 'fixed';
    scaleDiv.style.bottom = '40px';
    scaleDiv.style.left = '10px';
    scaleDiv.style.zIndex = '1000';
    scaleDiv.style.width = '150px';
    scaleDiv.style.display = 'none';

    const scaleSpan = document.createElement('span');
    scaleSpan.innerText = `Scale: ${scale}`;
    scaleSpan.className = 'font-bold';
    ...
    function convertToPrintVersion(scale = 0.75) {
    ...
    window.addEventListener('load', () => {
        ...
        let isHoveringPrintButton = false;

        printButton.addEventListener('mouseenter', () => {
            scaleDiv.style.display = 'block';
            isHoveringSlider = false;
            isHoveringPrintButton = true;
        });

        printButton.addEventListener('mouseleave', () => {
            isHoveringPrintButton = false;
            setTimeout(() => {
                if (!isHoveringSlider) {
                    scaleDiv.style.display = 'none';
                }
        ...
        scaleDiv.addEventListener('mouseenter', () => {
            scaleDiv.style.display = 'block';
            isHoveringSlider = true;
            isHoveringPrintButton = false;
        });
        scaleDiv.addEventListener('mouseleave', () => {
            isHoveringSlider = false;
            setTimeout(() => {
                if (!isHoveringPrintButton && scaleDiv) {
                    scaleDiv.style.display = 'none';
                }
            }, 200);
        });
        scaleDiv.addEventListener('input', () => {
            scale = parseFloat(scaleSlider.value);
            scaleSpan.innerText = `Scale: ${scale}`;
            localStorage.setItem('printScale', scale);
        });

        printButton.addEventListener('click', () => {
            convertToPrintVersion(scale);
            printButton.remove();
            scaleDiv.remove();
        });
    });
